    ...
    methods: {
            ...
            if (this.isConnected === true) {
                new BarcodeScanner().scan({
                    ...
                    .then(data => {
                        ...
                        setTimeout(() => {
                            if (this.isConnected === true) {    
                                if (is_rtpcr_scan) this.$showModal(ScannedRtpcr, { fullscreen: true, props: { qr_link: data.text }})
                                else {
                                    this.VERIFY_HDF({token: data.text})
                                        .then(response => {
                                            let passenger = response[0]
                                            passenger.updated_at = moment(passenger.updated_at).format('LLL')
                                            passenger.davao_arrival = passenger.davao_arrival && moment(passenger.davao_arrival).format('LL')
                                            passenger.davao_vessel_type = passenger.davao_vessel_type && passenger.davao_vessel_type.toUpperCase()
                                            passenger.davao_transport_type = passenger.davao_transport_type && passenger.davao_transport_type.toUpperCase()
                                            passenger.davao_vehicle_type = passenger.vehicle && passenger.vehicle.name.toUpperCase()
                                            this.hdf_scan( passenger.rtcpcr_verify )
                                            
                                            this.$showModal(ScannedHDF, { fullscreen: true, props: { passenger: passenger }})
                                        })
                                        .catch(response => {
                                            this.snackBar("facescan", response.data, "Qr code does not exist!")
                                        })
        ...
        calledIfHasNet () {
            ...
            confirm({
                ...
                message: "Click the Upload button to send your scanned QRs. Turn off mobile data or WiFi if this dialog message appears again.",
                okButtonText: "Upload",
        ...
        monitorNetworkStart() {
            ...
            startMonitoring((newConnectionType) => {
                ...
                case connectionType.none:
                    ...
                    this.snackBar("error", "Connection error", "Could not connect to server.")
